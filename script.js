...
function prevCadetStep() {
  document.getElementById("cadetStep2").classList.remove("active");
  document.getElementById("cadetStep1").classList.add("active");
}
...
function adminLogin() {
  ...
  if (!username || !password) {
    errorBox.textContent = "Please enter admin username and password";
    errorBox.style.display = "block";
    return;
  }

  // Match your fixed admin credentials
  if (username === "Arws2026" && password === "Arws@2026") {
    localStorage.setItem("loggedIn", "yes");
    localStorage.setItem("username", "Admin");
    localStorage.setItem("isAdmin", "yes");
